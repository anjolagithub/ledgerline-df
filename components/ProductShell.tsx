...
export function ProductShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    <header className="sticky top-0 z-30 border-b border-border bg-background/90 backdrop-blur-xl"><div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between px-5 lg:px-8"><Link href="/" className="flex items-center gap-3 text-xs font-semibold tracking-[.24em] text-slate-100"><span className="grid h-7 w-7 place-items-center bg-primary text-primary-foreground"><Activity size={14}/></span>LEDGERLINE</Link><button className="p-2 text-slate-400 lg:hidden" onClick={() => setOpen(!open)} aria-label="Toggle navigation">{open ? <X size={18}/> : <Menu size={18}/>}</button><nav className="hidden items-center gap-6 lg:flex">{nav.map(([label, href, Icon]) => <Link key={href} href={href} className={`flex items-center gap-2 font-mono text-[10px] uppercase tracking-[.1em] ${pathname === href ? 'text-primary' : 'text-slate-500 hover:text-slate-200'}`}><Icon size={13}/>{label}</Link>)}</nav></div>{open && <nav className="border-t border-border px-5 py-3 lg:hidden">{nav.map(([label, href, Icon]) => <Link key={href} href={href} onClick={() => setOpen(false)} className={`flex items-center gap-3 border-b border-border py-4 font-mono text-[10px] uppercase tracking-[.1em] ${pathname === href ? 'text-primary' : 'text-slate-400'}`}><Icon size={14}/>{label}</Link>)}</nav>}</header><main className="mx-auto max-w-[1440px]">{children}</main>
  </div>
}
