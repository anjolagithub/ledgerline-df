...
export default function Dashboard() {
  ...
  useEffect(() => { Promise.all([new JsonRpcProvider(SEPOLIA).getBlockNumber(), cc.getBlockNumber()]).then(() => { setSep(true); setCcStatus(true) }).catch(() => { setSep(false); setCcStatus(false) }) }, [cc])
  const lookup = async () => { if (!isAddress(query)) return setProfile('Invalid address'); setProfile('Reading profile…'); try { const p = await new Contract(REGISTRY, ABI, cc).getCreditProfile(query); setScore(String(p.score)); setProfile(`${formatUnits(p.totalVerifiedRepayments, 18)} verified volume · ${p.completedLoanCount} completed loans`) } catch { setProfile('No profile returned from CC3') } }
  ...
  return <main className="min-h-screen bg-background text-foreground">
    <header className="flex min-h-16 items-center justify-between border-b border-border px-5 lg:px-8"><Link href="/" className="flex items-center gap-3 text-xs font-semibold tracking-[.22em] text-slate-100"><span className="grid h-7 w-7 place-items-center bg-primary text-primary-foreground"><Network size={14} /></span>LEDGERLINE <span className="text-slate-600">/ CORE</span></Link><div className="flex items-center gap-5"><Status label="SEPOLIA" value={sep} /><Status label="CC3" value={ccStatus} /><button onClick={connect} className="glass flex items-center gap-2 border border-border px-3 py-2 font-mono text-[10px] uppercase tracking-[.1em] text-slate-300"><WalletCards size={13} />{wallet ? `${wallet.slice(0, 6)}…${wallet.slice(-4)}` : 'Connect wallet'}</button></div></header>
