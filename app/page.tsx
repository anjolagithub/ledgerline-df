        ...
        <div className="min-w-0 flex-1 px-4 py-6 sm:px-6 lg:px-10 lg:py-8">
          <div className="mx-auto max-w-[1440px]">
            ...
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
              ...
              <Card className="xl:col-span-5"><div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-4"><div className="flex items-center gap-2"><Server size={15} className="text-sky-400"/><h2 className="text-xs font-semibold text-slate-200">Verification monitor</h2></div><span className="flex items-center gap-1.5 text-[10px] text-emerald-400"><Dot/>Live</span></div><div className="divide-y divide-white/[0.05] px-5">{events.map((event) => <div key={event.id} className="flex items-center gap-3 py-3"><Dot tone={event.tone}/><div className="min-w-0 flex-1"><p className="truncate font-mono text-[10px] text-slate-300">{event.type}</p><p className="mt-0.5 font-mono text-[9px] text-slate-600">{event.id}</p></div><time className="font-mono text-[9px] text-slate-600">{event.time}</time></div>)}</div><div className="border-t border-white/[0.06] px-5 py-3"><button className="text-[10px] font-medium text-slate-500 hover:text-emerald-400">View all event logs <ArrowUpRight className="ml-1 inline" size={11}/></button></div></Card>
